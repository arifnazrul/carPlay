...
//  JD Driver
//
//  Created by Ajdari, Arian on 26.11.19.
...
/**
 Coastal Roads' map is displayed as a static image inside a UIScrollView, which enables panning and zooming.
 It supports only a single hardcoded route for navigation, and when the app is in navigation mode,
 the camera viewport animates between several pre-defined points on the map.
 In a real navigation app that uses a map with tiles, these locations would be provided at runtime
...
/**
 `MapScrollView` is a UIScrollView that displays a single static map image
 while still allowing for panning and zooming.

 In a real navigation app, this view could contain your map tiles.
 */
