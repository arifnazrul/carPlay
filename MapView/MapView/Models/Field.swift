...
class Field {
  ...
  init(filename: String) {
    ...
    guard let properties = Field.plist(filename) as? [String : Any],
        let boundaryPoints = properties["boundary"] as? [String] else { return }
    
    // assign the values to the class variables
    midCoordinate = Field.parseCoord(dict: properties, fieldName: "midCoord")
    overlayTopLeftCoordinate = Field.parseCoord(dict: properties, fieldName: "overlayTopLeftCoord")
    overlayTopRightCoordinate = Field.parseCoord(dict: properties, fieldName: "overlayTopRightCoord")
    overlayBottomLeftCoordinate = Field.parseCoord(dict: properties, fieldName: "overlayBottomLeftCoord")
